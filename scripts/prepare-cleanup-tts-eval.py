...
DEFAULT_OUTPUT = REPO / ".cache/stt-eval/personal-conversation-tts-v3-qwen3-ryan"
...
def selected_cases(suite: str):
    ...
    personal_v2 = REPO / "docs/evaluation/stt_personal_conversation_tts_cases_v2.jsonl"
    personal_v3 = REPO / "docs/evaluation/stt_personal_conversation_tts_cases_v3.jsonl"
    cases = []
    ...
    if suite == "personal-v2":
        cases.extend(project_additional_cases(personal_v2, REPO))
    if suite in {"personal-v3", "all-regressions"}:
        cases.extend(project_additional_cases(personal_v3, REPO))
    return cases
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--suite",
        choices=("personal-v3", "personal-v2", "heldout-v1", "seed", "all-regressions"),
        default="personal-v3",
    )
